<x-jet-form-section submit="update">
    ...
    <x-slot name="description">
        {{ __('Mail Setup is required to send out any communications from your system to your end users.') }}
    </x-slot>
    ...
    <x-slot name="form">
        <div class="col-span-6 sm:col-span-4">
            ...
            <div class="relative">
                <select id="mail" class="form-input border-gray-300 rounded-md shadow-sm mt-1 block w-full cursor-pointer" wire:model="state.mail.type">
                    ...
                    <option value="smtp">{{ _('SMTP') }}</option>
        ...
        <div class="col-span-6 sm:col-span-4 grid grid-cols-1 gap-6">
            <div>
                ...
                <x-jet-input id="host" type="text" class="mt-1 block w-full" wire:model.defer="state.mail.smtp.host" />
                <x-jet-input-error for="state.mail.smtp.host" class="mt-1 mb-2" />
            </div>
            <div>
                <x-jet-label for="port" value="{{ __('Port') }}" />
                <x-jet-input id="port" type="text" class="mt-1 block w-full" wire:model.defer="state.mail.smtp.port" />
                <x-jet-input-error for="state.mail.smtp.port" class="mt-1 mb-2" />
            </div>
            <div>
                <x-jet-label for="username" value="{{ __('Username') }}" />
                <x-jet-input id="username" type="text" class="mt-1 block w-full" wire:model.defer="state.mail.smtp.username" />
                <x-jet-input-error for="state.mail.smtp.username" class="mt-1 mb-2" />
            </div>
            <div x-data="{ show_password: false }">
                <x-jet-label for="password" value="{{ __('Password') }}" />
                <div class="relative">
                    <x-jet-input id="password" x-bind:type="show_password ? 'text' : 'password'" class="mt-1 block w-full" placeholder="{{ __('Enter the Password') }}" wire:model.defer="state.mail.smtp.password" autocomplete="new-password"/>
            ...
            <div>
                ...
                <div class="relative">
                    <select id="encryption" class="form-input border-gray-300 rounded-md shadow-sm mt-1 block w-full cursor-pointer" wire:model.defer="state.mail.smtp.encryption">
                        <option selected disabled value="empty">{{ __('Select a Encryption Type') }}</option>
                        <option value="tls">{{ _('TLS') }}</option>
                        <option value="ses">{{ _('SSL') }}</option>
