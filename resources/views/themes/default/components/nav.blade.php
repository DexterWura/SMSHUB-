<nav class="h-full">
    <div class="hidden lg:flex sticky top-0 z-40 h-24">
        <div class="w-full">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center">
                    <div class="flex items-baseline space-x-4">
                        ...
                        <form action="{{ route('logout') }}" method="post">
                            @csrf
                            <button type="submit" class="px-3 py-2 text-sm font-semibold text-left bg-transparent border-dashed border-2 border-gray-800 text-gray-700 rounded-lg md:w-auto md:inline hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none">
                                {{ __('Logout') }}
                            </button>
                        </form>
                        @endif
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4 items-center md:ml-6">
                        <div class="relative">
                            <form action="{{ route('locale', '') }}" id="locale-form" method="post">
                                @csrf
                                <select class="block appearance-none bg-white border-gray-300 cursor-pointer text-gray-800 py-1 rounded-md focus:outline-none focus:border-0 focus:shadow-none" name="locale" id="locale">
                                    @foreach(config('app.locales') as $index => $locale)
                                    <option value="{{ $locale }}" {{ (app()->getLocale() == $locale ) ? "selected" : "" }}>{{ config('app.locales_text')[$index] }}</option>
                                    @endforeach
                                </select>
                            </form>
                        </div>
    ...
    <div class="lg:hidden flex justify-end items-center h-full" x-data="{ open: false }">
        <div @click="open = true" class="flex items-center w-8 text-black">
                ...
                <div class="flex flex-col items-center justify-between">
                    <div class="flex flex-col items-center space-y-2">
                        @foreach($menus as $menu)
                            @if($menu->hasChild()) 
                            <div @click.away="open = false" class="relative" x-data="{ open: false }">
                                <button @click="open = !open" class="flex flex-row items-center w-full px-3 py-2 text-white font-semibold text-left bg-transparent rounded-lg md:w-auto md:inline focus:text-gray-900 focus:bg-gray-200 focus:outline-none">
                                    <span>{{ __($menu->name) }}</span>
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute left-0 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48">
                                    <div class="px-2 py-2 text-center bg-white rounded-lg shadow dark-mode:bg-gray-800">
                                        @foreach($menu->getChild() as $child)
                                        <a class="block font-semibold bg-transparent rounded-lg text-gray-600 md:mt-0 focus:text-gray-900 focus:bg-gray-100 focus:outline-none" href="{{ $child->link }}" target="{{ $child->target }}">{{ __($child->name) }}</a>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                            @else
                                @if($menu->parent_id === null)
                                <a href="{{ $menu->link }}" class="px-3 py-2 font-semibold text-left bg-transparent text-white {{ url()->current() === $menu->link ? 'bg-gray-200' : '' }} rounded-lg md:w-auto md:inline focus:text-gray-900 focus:bg-gray-200 focus:outline-none" target="{{ $menu->target }}">{{ __($menu->name) }}</a>
                                @endif
                            @endif
                        @endforeach
                        @if(Auth::check() && Auth::user()->role == 7)
                            <a href="{{ route('admin') }}" class="px-3 py-2 font-semibold text-left bg-transparent border-dashed border-2 border-red-400 text-red-400 rounded-lg md:w-auto md:inline hover:bg-red-100">{{ __('Admin') }}</a>
                        @endif
                        @if(Auth::check())
                        <form action="{{ route('logout') }}" method="post">
                            @csrf
                            <button type="submit" class="px-3 py-2 font-semibold text-left bg-transparent border-dashed border-2 border-gray-200 text-gray-100 rounded-lg md:w-auto md:inline">
                                {{ __('Logout') }}
                            </button>
                        </form>
                        @endif
                        ...
                        <div class="items-center mt-4">
                            <div class="relative">
                                <form action="{{ route('locale', '') }}" id="locale-form-mobile" method="post">
                                    @csrf
                                    <select class="block appearance-none bg-white border-gray-300 cursor-pointer text-gray-800 py-1 rounded-md focus:outline-none" name="locale" id="locale-mobile">
                                        @foreach(config('app.locales') as $index => $locale)
                                        <option value="{{ $locale }}" {{ (app()->getLocale() == $locale ) ? "selected" : "" }}>{{ config('app.locales_text')[$index] }}</option>
                                        @endforeach
                                    </select>
